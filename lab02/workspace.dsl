workspace {
    ...
    model {
        ...
        posts_system = softwareSystem "Система записей стен пользователя" {
            ...
            posts_db = container "База данных записей стен пользователей" {
            ...
            posts_service = container "Сервис обработки записей на стене" {
                ...
                technology "Python/FastAPI"
                -> posts_db "сохранение и получение записей стен пользователей" "JDBC"
            }
        }
        ...
        prod = deploymentEnvironment "PROD" {
            ...
            deploymentNode "Databases" {
                ...
                deploymentNode "Posts database Server" {
                    containerInstance posts_system.posts_db
                    instances 2
                }
    ...
    views {
        ...
        container posts_system {
            include user posts_system.posts_service posts_system.posts_db user_system.auth_service
            autoLayout
        ...
        dynamic social_network "Case4" {
            ...
            posts_system.posts_service -> posts_system.posts_db "Сохранение нового поста в базу данных"
        }
        ...
        dynamic social_network "Case5" {
            ...
            posts_system.posts_service -> posts_system.posts_db "Получает данные о записях на стене пользователя"
        }
