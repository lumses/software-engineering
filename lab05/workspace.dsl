workspace {
    ...
    model {
        ...
        user_system = softwareSystem "Система пользователей" {
            ...
            user_service = container "Сервис обработки пользователей" {
                ...
                -> user_cache "чтение и запись кэшированных данных пользователя" "Redis"
            }
        }
        ...
        posts_system = softwareSystem "Система записей стен пользователя" {
            ...
            post_db = container "База данных записей на стене" {
                description "Управляет сохранением и получением записей на стенах пользователей"
                technology "MongoDB"
            }
            ...
            posts_service = container "Сервис обработки записей на стене" {
                ...
                technology "Python/FastAPI"
                -> posts_system.post_db "сохранение и получение записей стен пользователей" "JDBC"
            }
        }
        ...
        prod = deploymentEnvironment "PROD" {
            ...
            deploymentNode "Redis Cache Server" {
                containerInstance user_system.user_cache
                instances 1
            }
            ...
            deploymentNode "Databases" {
                ...
                deploymentNode "Post database Server" {
                    containerInstance posts_system.post_db
                    instances 2
                }

                deploymentNode "Message database Server" {
    ...
    views {
        ...
        container user_system {
            include user user_system user_system.auth_service user_system.user_service user_system.user_db user_system.user_cache
            autoLayout
            title "Контекст пользователей"
        }

        container posts_system {
            include user posts_system.posts_service posts_system.post_db user_system.auth_service
            autoLayout
        ...
        dynamic social_network "Case4" {
            ...
            posts_system.posts_service -> posts_system.post_db "Сохранение нового поста в базу данных"
        }
        ...
        dynamic social_network "Case5" {
            ...
            posts_system.posts_service -> posts_system.post_db "Получает данные о записях на стене пользователя"
        }
